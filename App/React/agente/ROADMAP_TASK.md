...
> -   **Mejora Ayuno (v1.0.28):** Ventana de comida corregida: ahora es `periodo - objetivo` y comienza al terminar el ayuno (ej: objetivo 20h => ventana 4h). En el círculo inactivo se visualiza como reloj: arco verde = ventana de comida y marcador = inicio del próximo ayuno. Modal de finalizar muestra ventana y próximo ayuno con este cálculo.
...
> -   **Fix TypeScript (v1.0.32-beta):** Solucionados errores de tipo en componente `Boton` (prop `titulo` reemplazada por `title`) y configuración inválida en `tsconfig.json`. Corrección de etiqueta JSX mal cerrada en `ModalProyecto.tsx`.
> -   **Fix Estilos + Métricas Déficit (v1.0.33-beta):** Declarado módulo `*.css` para import side-effect en TypeScript (`assets.d.ts`), reemplazadas variables CSS inexistentes/hardcodeadas en `buscador.css`, `configuracionHabitos.css` y `proyectos.css`, añadidos aliases legacy en `variables.css` (`--dashboard-primario`, `--dashboard-error`, `--dashboard-exito`, `--dashboard-estadoError`, `--dashboard-sombraFlotante`) y corregido cálculo de fecha local en `useDeficitCalorico.ts` + `deficitCaloricoStore.ts` para que calorías/macros muestren solo comidas del día actual en hora local.
...
> -   **Hotfix Time Tracker (v1.0.40-beta):** En `DockTracking.tsx` se añadió control manual de tiempo con icono de reloj arrastrable (derecha suma, izquierda resta sin bajar de `00:00`) persistido en `timeTrackerStore.ts` mediante `ajusteManualMs` y acción `ajustarTiempoTracking`. Además, el título de la pestaña ahora se actualiza en tiempo real con `tiempo transcurrido + nombre de la tarea/hábito` mientras el tracker está activo/pausado, restaurando el título original al finalizar.
> -   **Ajuste de Fluidez Tracker (v1.0.41-beta):** Durante el arrastre del reloj en `DockTracking.tsx`, el cronómetro ahora se congela automáticamente (pausa temporal) para evitar que siga avanzando mientras se corrige el tiempo; al soltar, se reanuda solo si estaba activo antes. También se ajustó la sensibilidad del gesto para cambios más fluidos `segundo a segundo` y en `useTimeTracker.ts` se sincroniza `tiempoMs` ante cambios manuales de `sesionActiva` para reflejar cada segundo en vivo al mover.
