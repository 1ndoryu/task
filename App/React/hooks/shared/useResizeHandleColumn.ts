/*
 * useResizeHandleColumn
 * Lógica extraída de ResizeHandleColumn para cumplir SRP
 * Gestiona el arrastre para redimensionar columnas del dashboard
 */
...
interface UseResizeHandleColumnReturn {
    arrastrando: boolean;
    visible: boolean;
    ...
    claseArrastrando: string;
    titulo: string;
}

export function useResizeHandleColumn({tipo, posicion, modoColumnas, anchos, anchoTotal, onCambiarAnchos, onCambiarAnchoTotal}: UseResizeHandleColumnParams): UseResizeHandleColumnReturn {
    const [arrastrando, setArrastrando] = useState(false);

    /* Determinar visibilidad */
    const visible = !(tipo === 'interno' && modoColumnas === 1) && !(tipo === 'interno' && posicion === 2 && modoColumnas < 3);
