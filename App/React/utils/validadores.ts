...
export function validarHabitos(valor: unknown): valor is Habito[] {
    ...
    return valor.every(h => {
        ...
        return typeof habito.id === 'number' && typeof habito.nombre === 'string' && ['Muy Alta', 'Alta', 'Media', 'Baja', 'Muy Baja'].includes(habito.importancia as string) && typeof habito.diasInactividad === 'number' && typeof habito.racha === 'number' && Array.isArray(habito.tags);
    });
}
