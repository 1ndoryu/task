...
- [x] **H-F14-02** `MEDIA` `REGLA` — **32 archivos CSS superan el límite de 300 líneas**; los 8 monolíticos: `tareas.css` 794, `compartidos.css` 730, `tabla.css` 729, `panelGruposFb.css` 726, `panelAdministracion.css` 693, `bottomSheetCreacion.css` 648, `panelDeficitCalorico.css` 541, `chatHistorial.css` 448. **Resolver:** dividir por bloque funcional (p. ej. `tareas.css` → `tareas-lista.css` + `tareas-fila.css` + `tareas-acciones.css`) y dejar en cada archivo solo su sección.
  - ✅ 2026-08-19 (sesión 17): resuelto con criterio de duplicación real (no churn). Verificación: build `npm run build` OK con **hash CSS idéntico** (`index-B05DI39S.css`, 553.84 kB) antes/después + verificación semántica por script (91+99 selectores, 0 diferencias). **2 refactorizados por duplicación/cohesión real:** `bottomSheetCreacion.css` 648→216 (las 4 copias base+tarea+hábito+proyecto colapsadas en selectores agrupados + overrides de variante) y `panelAdministracion.css` 693→505 (feature feedback extraída a `admin/listaFeedback.css` 190, import en cascada actualizado). **6 documentados como justificados** (extensos pero cohesivos — un solo componente/feature, sus repetidos son one-liners normales, dividir sería churn sin reducir cascada): `tareas.css` 791 (familia tareaItem), `compartidos.css` 730 (feature modal compartir), `tabla.css` 729 (TablaHabitos), `panelGruposFb.css` 726 (panel recién refactorizado en F13), `panelDeficitCalorico.css` 541, `chatHistorial.css` 448. Además se limpiaron 2 hex residuales muertos en comentarios de `tareas.css` (los únicos hardcodeos restantes eran líneas comentadas).
