...
- [x] **H-F12-01** `ALTA` `REGLA` — **64 de 146 hooks superan el límite de 120 líneas** (regla 8). Peores: `useTareas` 507, `useDashboardApi` 494, `useConfiguracionLayout` 447, `useWebSocket` 428, `useDashboardSync` 406, `useSyncManager` 404, `useModoOffline` 396, `useModalHabito` 396, `useBackButtonCapacitor` 364, `useActividad` 352, `useDashboardHabitos` 348, `useScratchpad` 348, `useSincronizacion` 341, `useHabitosComoTareas` 336, `useModalesDashboard` 331, `useEquipos` 327. **Resolver:** dividir por responsabilidad en hooks especializados + utils; los que sean registro/configuración declarativa pueden justificar `sentinel-disable-file` (regla 14), pero solo `useTareas` lo declara hoy.
  ...
  - ✅ Resuelto por criterio 2026-08-19 (decisión del usuario): **el límite de 120 líneas se reinterpreta para hooks** — el anti-patrón real es la multi-responsabilidad y el tamaño extremo, no la longitud en sí. Revisados los 15 más grandes a fondo y el resto por señales estructurales (nº de callbacks/effects/memos y delegación a utils):
    - **Justificados por single-responsibility** (≈78 de 87): superficies de configuración/estado con callbacks de una línea que ya delegan lógica pura a utils — `useConfiguracionLayout` 456 (22 callbacks; lógica en `layoutFactory`/`layoutLogica`), `useModalesDashboard` 331 (20 callbacks, registro de modales), `useCompartidos` 358, `useAdministracion` 313, `useEquipos` 327, `useActividad` 352, `useHabitosComoTareas` 336 (3 memos puros), `useModalHabito` 396 (form-modal), `useBackButtonCapacitor` 292 (ya refactorizado a lista declarativa), `useModoOffline` 444 (IndexedDB/reintentos ya delegados en T3, H-F12-08/H-F12-10), `useWebSocket` 429 (máquina de estados de conexión: backoff/heartbeat/inactividad — cohesiva), más form-modales, derivaciones y selectores de 120–260 (useTema, useLocalStorage, useNotificaciones, useListaTareas, useTablaHabitos…). Forzarlos a <120 añade indirección sin ganancia de SRP (sub-hooks con 20 props).
    - **Candidatos reales → plan dedicado (T7 — cluster de sincronización)**: comparten el mismo flujo de datos y se refactorizan coordinados, no por separado — `useDashboardApi` 451 (mappers de contrato → utils), `useSyncManager` 405, `useDashboardSync` 407, `useSincronizacion` 343, `useSincronizacionTiempoReal` 253, `useNotificadorCambiosWebSocket` 315, `generadoresPropsPanel` 370. Registrados en `Agente/planes/00-PLAN-RESOLUCION.md` (T7) y en `roadmap.md`.
    - **Regla 8 del AGENTS.md** queda señalada para revisión del usuario: proponer ~300 líneas para hooks de una sola responsabilidad y dividir solo con multi-responsabilidad o >~450 líneas.
