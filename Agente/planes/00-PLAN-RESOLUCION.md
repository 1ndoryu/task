...
### T7 — Cluster de sincronización (refactor coordinado) ✅ (completado 2026-08-25, verificado 2026-09-02)
...
**Ejecutado como H-F12-13 (2026-08-25, evidencia en `Agente/completados/tareas-2026-08-25.md`; commits `46a27fb`, `27cce4d`, `6fb17cf`, `3a0eb78`):** mappers `tareaARequest`/`proyectoARequest`/`habitoARequest` → `utils/mappersContrato.ts`; `useOnlineStatus`/`obtenerNonce` → `hooks/useOnlineStatus.ts` (re-exportados desde `useDashboardApi`); helpers de bienvenida/guards → `utils/syncAyudas.ts`; `generateBackup` tipado (elimina 3 `@ts-ignore`; quedan 0 `@ts-ignore`/`as any` en el cluster); `useCallbacksWebSocket` extraído de `useDashboardSync`; `generadoresPropsLigeros` extraído de `generadoresPropsPanel`; `useSyncTransport` separado. Decisión documentada: la máquina init/auto-save de `useSyncManager` NO se partió — ambas fases comparten `syncMeta`/`hasChanges`/guards; separarlas elevaría ~10 refs/props y reintroduciría riesgo de desincronización (cohesión actual <450).

**Verificación 2026-09-02 (re-chequeo de cierre):** tamaños actuales `useDashboardApi` 387, `useSyncManager` 404, `useDashboardSync` 305 (+192 `useCallbacksWebSocket`), `useSincronizacion` 344, `useSincronizacionTiempoReal` 255, `useNotificadorCambiosWebSocket` 315, `generadoresPropsPanel` 259 (+137 `generadoresPropsLigeros`) — todos con responsabilidad única y <450 (criterio H-F12-01). API pública intacta (barrels en `hooks/index.ts` y `useDashboardGrid.ts` sin cambios). Type-check exit 0 en el commit del cierre y sentinel sin hallazgos nuevos. T7 CERRADO.
