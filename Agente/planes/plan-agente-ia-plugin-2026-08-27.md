...
### Fase 3 — Memoria y automejora (MEMORIA v1 ✅ commit `306954a`; SKILLS CRUD + inyección ✅ `9113b4d`; UI de skills ✅ commit de este bloque; tsvector/automejora: iteración pendiente)
...
- **UI de administración de skills v1** ✅ (este bloque): sección "Skills" en el `ModalConfigAgente` — listar, crear, editar, activar/desactivar y eliminar, conectada a los endpoints CRUD autenticados (`/agente/skills`) con el usuario de la sesión actual; el flag `incluir_skills` de la config por conversación sigue controlando la inyección en el stream. Descripción requerida (label no "opcional") para cuadrar el contrato backend (1–4000). Verificado en vivo por la UI + E2E caso (create/edit/upsert/validation/injection/deactivate/delete).
- Pendiente (iteración posterior, requiere LLM estable y más backend): búsqueda con `tsvector` y automejora post-turno (modelo barato solo local/dev) que **sugiere** skills.
...
- [x] CRUD manual de skills con topes y DPI — backend v1 hecho y verificado por E2E; la UI del modal también hecha (este bloque).
