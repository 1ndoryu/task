...
import {ETIQUETAS_DIFICULTAD, opcionesMenuDificultad, type DificultadNivel} from '../../utils/nivelesConfig';
...
export function SelectorDificultadPill({dificultad, onChange, deshabilitado = false}: SelectorDificultadPillProps): JSX.Element {
    ...
    return (
        <div className="propiedadesCompactas__item">
            <Boton ref={botonRef} type="button" variante="ghost" claseAdicional={`pillOpcion ${dificultad === 'Media' ? 'pillOpcion--vacio' : `pillOpcion--dificultad-${dificultad.toLowerCase().replace(/ /g, '')}`} ${deshabilitado ? 'pillOpcion--disabled' : ''}`} onClick={abrirMenu} title="Dificultad">
